...
#ifdef CUDA
#if 0
    //module_cuda_timestamp_queue_update_interface(tid, &gts.thread[tid]);
    //module_cuda_global_timestamp_queue_update_interface(&gts);
#endif
#endif
}
    ...
    for (i = 0; i < syn.mutex.count; i++) {
        if (lock_id == syn.mutex.entry[i].id) {
            ...
            module_timestamp_merge_two(syn.mutex.entry[i].last_unlock_tid, 
                    &gts.thread[syn.mutex.entry[i].last_unlock_tid].entry[syn.mutex.entry[i].last_unlock_ts_index], 
                    tid, &cts.ts[tid]);
            ...
            for (k = 0; k < MAX_PROCESS_NUM; k++) {
                if (syn.cond.entry[i].is_require[k]) {
                     module_timestamp_merge_two(k, 
                            &gts.thread[k].entry[syn.cond.entry[i].last_cond_ts_index[k]], 
                            tid, &cts.ts[tid]);
                }
            }
        }
            ...
            if (syn.barrier.entry[i].is_barrier >= max_thread_num) {
                ...
                for (k = 0; k < MAX_PROCESS_NUM; k++) {
                    if (syn.barrier.entry[i].is_require[k]) {
                        ts1[k] = &gts.thread[k].entry[syn.barrier.entry[i].last_barrier_ts_index[k]];
                        ts2[k] = &cts.ts[k];
                    }
                }
...
void helper_syn_create_trace(void) {
    uint8_t tid;
    uint32_t index;
...
void helper_syn_join_trace(void) {
    uint8_t tid;
    uint32_t index;
...
static inline void helper_syn_clone_trace(void) {
    ...
    module_timestamp_merge_two(parent_tid, 
            &gts.thread[parent_tid].entry[syn.thread.create_ts_index], 
            tid, &cts.ts[tid]);

    stat_syn.clone_count++;
}

static inline void helper_syn_exit_trace(void) {
    uint8_t tid, parent_tid;
    uint32_t index;
    ...
    module_timestamp_merge_two(tid, 
            &gts.thread[tid].entry[syn.thread.exit_ts_index[tid]], 
            parent_tid, &cts.ts[parent_tid]);
...
void helper_syn_condbroad_trace(target_ulong pc) {
    ...
    for (i = 0; i < syn.cond.count; i++) {
        if (cond_id == syn.cond.entry[i].id) {
            ...
            for (k = 0; k < MAX_PROCESS_NUM; k++) {
                if (syn.cond.entry[i].is_require[k]) {
                    ts1[k] = &gts.thread[k].entry[syn.cond.entry[i].last_cond_ts_index[k]];
                    ts2[k] = &cts.ts[k];
                }
            }
