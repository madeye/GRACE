...
#ifdef PARALLEL_STAGE_2
        if (temp_chunk->info->is_buf_full_flags[info.core_id]) {
#else
...
#ifdef PARALLEL_STAGE_2
            temp_chunk->info->is_buf_full_flags[info.core_id] = 0;	
#else
...
#ifdef PARALLEL_STAGE_2
        module_shared_buf_copy_2(0, 0, info.chunk_id, tid, TRACE_SHARED_BUF_SIZE, buf_ptr);
#else
...
#ifdef PARALLEL_STAGE_2
    module_shared_buf_copy_2(0, 0, info.chunk_id, tid, buf_size, buf_ptr);
#else
